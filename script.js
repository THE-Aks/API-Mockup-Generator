...
(function () {
    ...
    function updateStats() {
        el.statEndpoints.textContent = state.endpoints.length;
        el.statSchemas.textContent = state.schema.length;
        el.statObjects.textContent = state.mockData.length;
        el.statRequests.textContent = state.requests.length;

        if (!state.validation) {
            el.statValidation.textContent = '—';
            el.statValidation.style.color = '';
        } else if (state.validation.valid) {
            el.statValidation.textContent = 'Valid';
            el.statValidation.style.color = 'var(--success)';
        } else {
            el.statValidation.textContent = 'Failed';
            el.statValidation.style.color = 'var(--danger)';
        }
    }
    ...
    function tsType(fieldType) {
        switch (fieldType) {
            case 'number': case 'price': return 'number';
    ...
    function generateSdkJs() {
        const resource = state.resourceName || 'items';
        const Resource = toPascalCase(resource);
        return `// ${Resource} API client — generated by API Mockup Generator
const BASE_URL = "https://mockapi.local/api/${resource}";
...
`;
    ...
    function generateSdkTs() {
        const resource = state.resourceName || 'items';
        const Resource = toPascalCase(resource);
        const fields = state.schema.length ? state.schema : DEFAULT_SCHEMA;
        const interfaceBody = fields.map((f) => `  ${f.name}${f.required ? '' : '?'}: ${tsType(f.type)};`).join('\n');
...
export async function getAll${Resource}(): Promise<${Resource}[]> {
  const response = await fetch(BASE_URL);
  if (!response.ok) throw new Error("Failed to fetch ${resource}");
  return response.json();
}
...
export async function create${Resource}(payload: Partial<${Resource}>): Promise<${Resource}> {
  const response = await fetch(BASE_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload)
...
export async function update${Resource}(id: string, payload: Partial<${Resource}>): Promise<${Resource}> {
  const response = await fetch(\`\${BASE_URL}/\${id}\`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload)
...
export async function delete${Resource}(id: string): Promise<{ success: boolean }> {
  const response = await fetch(\`\${BASE_URL}/\${id}\`, { method: "DELETE" });
  if (!response.ok) throw new Error("Failed to delete ${resource} " + id);
  return response.json();
}
`;
    ...
    function generateSdkReact() {
        const resource = state.resourceName || 'items';
        const Resource = toPascalCase(resource);
        return `// use${Resource}.js — React hook generated by API Mockup Generator
...
export function use${Resource}() {
  ...
  const create${Resource} = useCallback(async (payload) => {
  ...
  return { data, loading, error, refresh, create${Resource} };
}
`;
    }
    ...
    function renderSdk() {
        let code;
        if (activeSdkTab === 'typescript') code = generateSdkTs();
        else if (activeSdkTab === 'react') code = generateSdkReact();
        else code = generateSdkJs();

        el.sdkCode.innerHTML = `<code>${escapeHtml(code)}</code>`;
    }
    ...
    function copySdk() {
        ...
        navigator.clipboard.writeText(code)
            .then(() => showToast('SDK code copied to clipboard', 'success'))
            .catch(() => showToast('Copy failed — try manually selecting the text', 'error'));
    }

    // Demo
    function runDemo() {
        document.querySelector('.tab-btn[data-tab="nl"]').click();
        el.nlInput.value = `Create an API for an online bookstore.
...
stock`;
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                el.mockCount.value = 12;
                generateMockData();
                setTimeout(() => {
                    document.querySelector('.tab-btn[data-otab="tester"]').click();
                    setTimeout(() => sendTestRequest(), 300);
                }, 500);
            }, 500);
        }, 600);
    }
    ...
    // Initialization
    function init() {
        // seed with a sensible default schema so the app never looks empty
        state.schema = DEFAULT_SCHEMA.map((f) => ({ ...f }));
        renderSchemaTable();
        regenerateEndpoints();
        renderSdk();
        ...
        $('#btnParseNL').addEventListener('click', (e) => withLoading(e.currentTarget, parseNaturalLanguage));
        $('#btnParseOpenAPI').addEventListener('click', (e) => withLoading(e.currentTarget, parseOpenApi));
        $('#btnParsePostman').addEventListener('click', (e) => withLoading(e.currentTarget, parsePostman));
        $('#btnAddField').addEventListener('click', addField);
        $('#btnGenerateMock').addEventListener('click', (e) => withLoading(e.currentTarget, generateMockData, 650));
        $('#btnCopyJson').addEventListener('click', copyJson);
        $('#btnDownloadJson').addEventListener('click', downloadJson);
        $('#btnSend').addEventListener('click', sendTestRequest);
        $('#btnCopySdk').addEventListener('click', copySdk);
        $('#btnGenerateHero').addEventListener('click', () => {
            $('#dashboard').scrollIntoView({ behavior: 'smooth' });
        });
        $('#btnViewDemo').addEventListener('click', runDemo);

        el.resourceNameInput.addEventListener('input', (e) => {
            state.resourceName = e.target.value.trim().toLowerCase().replace(/\s+/g, '_') || 'items';
            regenerateEndpoints();
        });

        el.mockCount.addEventListener('change', () => {
            let v = parseInt(el.mockCount.value, 10) || 1;
            v = Math.min(100, Math.max(1, v));
            el.mockCount.value = v;
        });
    }
